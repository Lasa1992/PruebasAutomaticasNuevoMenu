...
public class FacturacionConceptoSustitucion {
    ...
    public void FacturacionporConcepto() {
        ...
        BotonAgregarListado(); // Abre el formulario de facturación
        ...
        ObtenerFolioFactura(); // Obtiene el folio de la factura generada
        AceptarFactura(); // Acepta la factura
        BotonTimbre(); // Timbrar la factura
        ValidarYEnviarCorreo();// Validar posibles errores
        BotonPoliza(); // Botón de póliza
        BotonImpresion(); // Botón de impresión
        ...
        MonedaSustitucion(); // Filtra la moneda para que concuerde con la del viaje.
    ...
    public void ObtenerFolioFactura() {
        try {
            // Esperar hasta que el campo de factura con la clase específica sea visible
            WebElement campoFactura = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[contains(@class, 'EDT_FOLIO')]")));

            // Obtener el valor del campo
            FolioFactura = campoFactura.getAttribute("value").trim();

            // Imprimir el valor obtenido en consola
    ...
    private static void BusquedaFacturaListado() {
        try {
            // Espera hasta que el campo de búsqueda sea visible
    ...
    private static void SeleccionarFactura() {
        try {
            // Espera de 3 segundos antes de empezar
            Thread.sleep(3000);

            // Esperar a que la tabla de facturas esté visible
            WebElement tablaFacturas = wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("TABLE_ProFacturasPorConcepto")));

            // Buscar la fila que contiene el número de factura dentro de la primera celda
            ...
            // Hacer scroll hasta la fila para asegurar que sea visible
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({block: 'center'});", fila);

            // Intentar hacer clic en la fila
            try {
                fila.click();
            } catch (Exception e) {
                // Si el clic normal falla, usar JavaScriptExecutor como respaldo
            ...
            System.out.println("⚠️ ERROR: No se encontró la factura con folio: " + FolioFactura);
            UtilidadesAllure.manejoError(driver, e, "No se encontró la factura con el número: " + FolioFactura);
        } catch (TimeoutException e) {
            System.out.println("⚠️ ERROR: La tabla no cargó los resultados a tiempo.");
            UtilidadesAllure.manejoError(driver, e, "La tabla de facturas no cargó correctamente.");
        } catch (Exception e) {
            System.out.println("⚠️ ERROR: Ocurrió un problema al seleccionar la factura.");
